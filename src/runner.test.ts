...
describe("runCursorAgent", () => {
  ...
  it("classifies stderr-only failure", async () => {
    const proc = createMockChildProcess();
    spawnMock.mockReturnValue(proc);
    ...
    const result = await promise;
    expect(result.success).toBe(false);
    expect(result.errorClass).toBe("stderr_error");
  });
  ...
  it("passes --force only when mcpApprovalMode=force", async () => {
    const proc = createMockChildProcess();
    spawnMock.mockReturnValue(proc);

    const promise = runCursorAgent(makeOpts({ enableMcp: true, mcpApprovalMode: "force" }));
    proc.simulateClose(0);
    await promise;
